...
@RegisterEvent
export class CompetitionFinished extends Event<CompetitionEntity> {
  public EventType: EventType = EventType.COMPETITION_FINISHED;
  ...
  load(data: IEventJSON<CompetitionEntity>): Event<CompetitionEntity> {
    const ev = new CompetitionFinished(data.payload);
    return ev;
  }
